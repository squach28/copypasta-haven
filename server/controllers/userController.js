...
export const removePostFromDislikes = async (req, res) => {
    try {
        await User.findOneAndUpdate({
            _id: req.query.userId,
        }, { $pull: { dislikes: req.query.postId }})
        res.status(200).json({ success: true, message: 'Post was removed from dislikes'})
    } catch(err) {
        console.log(err)
    }
}
